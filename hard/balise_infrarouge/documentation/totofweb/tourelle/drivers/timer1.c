...
void (*timer1_isr[4])(void) = {T1_ISR_NULLFUNC, T1_ISR_NULLFUNC, T1_ISR_NULLFUNC, T1_ISR_NULLFUNC};
volatile unsigned char timer1_isr_cnt[4] = {0, 0, 0, 0};
...
#ifdef TIMER1_OVF_NOBLOCK
ISR(TIMER1_OVF_vect, ISR_NOBLOCK) {				// Interruption sur overflow de TCNT1
#else
ISR(TIMER1_OVF_vect) {							// Interruption sur overflow de TCNT1
#endif
...
#ifdef TIMER1_COMPA_NOBLOCK
ISR(TIMER1_COMPA_vect, ISR_NOBLOCK) {			// Interruption sur Compare Match (TCNT1 = OCR1A)
#else
...
#ifdef TIMER1_COMPB_NOBLOCK
ISR(TIMER1_COMPB_vect, ISR_NOBLOCK) {			// Interruption sur Compare Match (TCNT1 = OCR1B)
#else
...
#ifdef TIMER1_COMPC_NOBLOCK
ISR(TIMER1_COMPC_vect, ISR_NOBLOCK) {			// Interruption sur Compare Match (TCNT1 = OCR1C)
#else
ISR(TIMER1_COMPC_vect) {						// Interruption sur Compare Match (TCNT1 = OCR1C)
#endif
	timer1_isr_cnt[(unsigned char)T1_ISR_COMP_C]++;
	if (timer1_isr[(unsigned char)T1_ISR_COMP_C])
		(*timer1_isr[(unsigned char)T1_ISR_COMP_C])();
}
