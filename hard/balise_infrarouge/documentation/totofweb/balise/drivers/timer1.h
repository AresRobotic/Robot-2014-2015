...
typedef enum {					// Mode d'opération du timer
	...
	T1_MODE_PWM_FAST_TOPICR			= ((1<<WGM13)|(1<<WGM12)|(1<<WGM11)|(0<<WGM10)),
...
extern void (*timer1_isr[3])(void);
extern volatile unsigned char timer1_isr_cnt[3];
...
void timer1ctc_isr_enable(void (*func)(void)) {
	...
	sbi(TIFR, OCF1A);			// Reset du flag d'interruption
	sbi(TIMSK, OCIE1A);
}
